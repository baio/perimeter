...
module MultiUsers =
    ...
    type ``permissions-multi-users-api``(testFixture: TestFixture, output: ITestOutputHelper) =
        ...
        member __.``user 1 forbidden to create permission in user 2 tenant``() =
            ...
            task {
                let data: PostLike =
                    { Name = "test:permissions"
                      Description = "test description" }
                let! result = testFixture.HttpPostAsync u1.Token.Value (sprintf "/tenant/apis/%i/permissions" u2.Tenant.Value.SampleApiId) data
        ...
        member __.``user 1 forbidden to update permission in user 2 tenant``() =
            ...
            task {
                let data: PostLike =
                    { Name = "test:permissions"
                      Description = "test description" }
                let! result = testFixture.HttpPutAsync u1.Token.Value (sprintf "/tenant/apis/%i/permissions/%i" u2.Tenant.Value.SampleApiId u2.PermissionId.Value) data
                ensureForbidden result
            }
        ...
        member __.``user 1 forbidden to get permission in user 2 tenant``() =
            ...
            let u2 = users.[1]
            task {
                let! result = testFixture.HttpGetAsync u1.Token.Value (sprintf "/tenant/apis/%i/permissions/%i" u2.Tenant.Value.SampleApiId u2.PermissionId.Value)
                ensureForbidden result
            }
        ...
        member __.``user 1 forbidden to delete permission in user 2 tenant``() =
            ...
            let u2 = users.[1]
            task {
                let! result = testFixture.HttpDeleteAsync u1.Token.Value (sprintf "/tenant/apis/%i/permissions/%i" u2.Tenant.Value.SampleApiId u2.PermissionId.Value)
                ensureForbidden result
            }
