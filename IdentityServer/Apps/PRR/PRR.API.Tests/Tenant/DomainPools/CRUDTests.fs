    ...
    type ``domain-pool-crud-api``(testFixture: TestFixture, output: ITestOutputHelper) =
        ...
        member __.``A Create domain pool must be success``() =
            task {
                ...
                let! result =
                    testFixture.Server2.HttpPostAsync userToken (sprintf "/api/tenants/%i/domain-pools" tenantId.Value) data
        ...
        member __.``C.2 Get domains list be success``() =
            task {
                let! result = testFixture.Server2.HttpGetAsync userToken (sprintf "/api/tenants/%i/domain-pools" tenantId.Value)
                do! ensureSuccessAsync result
