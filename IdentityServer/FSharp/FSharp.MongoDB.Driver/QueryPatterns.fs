...
module Query =
    ...
    let rec internal parser v q =
        let (|Comparison|_|) op =
            function
            | InfixOp op (GetField (var, field), ValueOrList (value, _)) when var = v -> Some(field, value)
            | _ -> None

        match q with
        | Comparison <@ (=) @> (field, value) -> Some(BsonElement(field, BsonValue.Create value))

        | InfixOp <@ (=) @> (InfixOp <@ (%) @> (GetField (var, field), Int32 (divisor)), Int32 (remainder)) when var = v ->
            Some(BsonElement(field, BsonDocument("$mod", BsonArray([ divisor; remainder ]))))

        | Comparison <@ (<>) @> (field, value) -> Some(BsonElement(field, BsonDocument("$ne", BsonValue.Create value)))

        | Comparison <@ (>) @> (field, value) -> Some(BsonElement(field, BsonDocument("$gt", BsonValue.Create value)))

        | Comparison <@ (>=) @> (field, value) -> Some(BsonElement(field, BsonDocument("$gte", BsonValue.Create value)))

        | Comparison <@ (<) @> (field, value) -> Some(BsonElement(field, BsonDocument("$lt", BsonValue.Create value)))

        | Comparison <@ (<=) @> (field, value) -> Some(BsonElement(field, BsonDocument("$lte", BsonValue.Create value)))
        ...
        | InfixOp <@ (=~) @> (GetField (var, field), String (pcre)) when var = v ->
            ...
            Some
                (BsonElement
                    (field,
                     BsonDocument
                         ([ BsonElement("$regex", BsonString(regex))
        ...
        | SpecificCall <@ not @> (_, _, [ expr ]) ->
            ...
            | Some elem -> Some(BsonElement("$not", elem.Value))
            // Some { elem with Value = BsonDocument("$not", elem.Value) }
            | None -> None
            ...
            | Some lhsElem ->
                ...
                | Some rhsElem ->
                    if lhsElem.Name = "$and" then
                        lhsElem.Value.AsBsonArray.Add(toDoc rhsElem)
                        |> ignore
                        Some lhsElem

                    else
                        Some(BsonElement("$and", BsonArray([ lhsElem; rhsElem ] |> Seq.map toDoc)))
            ...
            | Some lhsElem ->
                ...
                | Some rhsElem ->
                    if lhsElem.Name = "$or" then
                        ...
                        Some(BsonElement("$or", BsonArray([ lhsElem; rhsElem ] |> Seq.map toDoc)))
